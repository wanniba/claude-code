...
function ModelPicker({ onDone }: { onDone: LocalJSXCommandOnDone }) {
  ...
  function save(provider: ProviderConfig, model: string, key: string, url: string) {
    saveGlobalConfig((c) => ({
      ...c,
      customModelProvider: {
    ...
    onDone(`✓ 已切换到 ${provider.label} · ${model}`, { display: "system" });
  }
  ...
  return (
    <Box flexDirection="column" paddingY={1}>
      ...
      {step === "select-model" && (
        <Box flexDirection="column">
          <Text dimColor>{selectedProvider.label} · ↑↓ 选择，Enter 确认，Esc 返回</Text>
          <Box flexDirection="column" marginTop={1}>
